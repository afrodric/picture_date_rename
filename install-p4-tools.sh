...
function do_faricp4_test {
   

    cd ${BUILD_DIR}
    if [ ! -d fabric-p4test ]; then
        git clone https://github.com/opennetworkinglab/fabric-p4test.git fabric-p4test
    fi
    cd fabric-p4test
    git pull origin master
...
check_and_do ${FABRICP4_TEST_COMMIT} fabric-p4test do_faricp4_test fabric-p4test
